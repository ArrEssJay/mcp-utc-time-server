...
const TEST_PORT: u16 = 13000; // Use different port to avoid conflicts
const BASE_URL: &str = "http://127.0.0.1:13000";
...
async fn test_health_endpoint() {
    ...
    assert!(body.contains("healthy"), "Response should contain 'healthy'");
...
async fn test_api_unix_endpoint() {
    ...
    assert!(seconds > 1_600_000_000, "Unix timestamp should be after 2020");
}
...
async fn test_api_nanos_endpoint() {
    ...
    assert!(json.get("nanoseconds").is_some(), "Should have 'nanoseconds' field");
    assert!(json.get("seconds").is_some(), "Should have 'seconds' field");
    assert!(json.get("subsec_nanos").is_some(), "Should have 'subsec_nanos' field");
}
...
async fn test_api_timezones_endpoint() {
    ...
    assert!(response.is_ok(), "GET /api/timezones failed: {:?}", response);
    
    let body = response.unwrap();
    let json: serde_json::Value = serde_json::from_str(&body).expect("Invalid JSON");
    
    assert!(json.get("timezones").is_some(), "Should have 'timezones' field");
    assert!(json.get("count").is_some(), "Should have 'count' field");
    
    let timezones = json["timezones"].as_array().expect("timezones should be array");
    ...
    let tz_strings: Vec<String> = timezones
        ...
        .collect();
    assert!(tz_strings.contains(&"UTC".to_string()), "Should include UTC");
    assert!(tz_strings.contains(&"America/New_York".to_string()), "Should include America/New_York");
}
...
async fn test_api_timezone_specific() {
    ...
    assert!(response.is_ok(), "GET /api/time/timezone/America/New_York failed: {:?}", response);
    
    let body = response.unwrap();
    let json: serde_json::Value = serde_json::from_str(&body).expect("Invalid JSON");
    
    assert!(json.get("timezone").is_some(), "Should have 'timezone' field");
    assert_eq!(json["timezone"].as_str(), Some("America/New_York"), "Timezone should match");
}
...
async fn test_api_ntp_status_container_mode() {
    ...
    assert!(response.is_ok(), "GET /api/ntp/status failed: {:?}", response);
    
    let body = response.unwrap();
    let json: serde_json::Value = serde_json::from_str(&body).expect("Invalid JSON");
    
    // In container mode, NTP should not be available
    assert_eq!(json["available"], false, "NTP should not be available in container mode");
    assert_eq!(json["container_mode"], true, "Should indicate container mode");
}
...
async fn test_metrics_endpoint() {
    ...
    let body = response.unwrap();
    assert!(body.contains("mcp_time_seconds"), "Should contain mcp_time_seconds metric");
    assert!(body.contains("mcp_time_nanos"), "Should contain mcp_time_nanos metric");
    assert!(body.contains("# TYPE"), "Should contain Prometheus TYPE directives");
}
...
async fn test_404_endpoint() {
    ...
    assert_eq!(response.status(), 404, "Should return 404 for unknown endpoint");
    
    let body = response.text().await.expect("Failed to read body");
    let json: serde_json::Value = serde_json::from_str(&body).expect("Invalid JSON");
    
    assert!(json.get("error").is_some(), "Should have 'error' field");
    assert!(json.get("available_endpoints").is_some(), "Should list available endpoints");
}
...
async fn test_cors_headers() {
    ...
    let url = format!("{}/api/time", BASE_URL);
    let response = reqwest::get(&url).await.expect("Request failed");
